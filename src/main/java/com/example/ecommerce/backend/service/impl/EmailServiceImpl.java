...
import com.example.ecommerce.backend.config.AppEmailProperties;
import com.example.ecommerce.backend.dto.response.OrderItemResponse;
...
public class EmailServiceImpl implements EmailService {
    ...
    private static final Pattern NAMED_EMAIL_PATTERN = Pattern.compile("^.+\\s<[^\\s@]+@[^\\s@]+\\.[^\\s@]+>$");
    ...
    public void sendOrderEmail(String to, OrderResponse order, OrderEmailEventType type) {
        if (!props.enabled() || !isValidRequest(to, order, type) || !hasApiConfig())
            return;

        ResendEmailRequest payload = new ResendEmailRequest(
                props.from().trim(),
                to,
                buildSubject(order, type),
                buildBody(order, type));

        try {
            restClient.post()
                    .uri(normalizeBaseUrl(props.apiBaseUrl()) + "/emails")
            ...
            log.error("Failed to send order email via Resend API to {}", to, ex);
        }
    }
    ...
    private boolean hasApiConfig() {
        boolean valid = props.from() != null
                && !props.from().isBlank()
                && props.apiKey() != null
                && !props.apiKey().isBlank();
        if (!valid) {
            log.warn("Email disabled due to missing Resend config (app.email.from or app.email.api-key)");
            return false;
        }
        if (!isValidFromAddress(props.from().trim())) {
            log.warn("Invalid app.email.from format: '{}'. Expected 'email@example.com' or 'Name <email@example.com>'",
                    props.from());
            return false;
        }
        return true;
    ...
    private boolean isValidFromAddress(String from) {
        return SIMPLE_EMAIL_PATTERN.matcher(from).matches() || NAMED_EMAIL_PATTERN.matcher(from).matches();
    }
